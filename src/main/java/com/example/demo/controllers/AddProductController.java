...
// make the add and remove buttons work
 ...
 //        return "confirmationassocpart";
    ...
    public String buyProduct(@RequestParam("productID") int theID) {
        ProductService productService = context.getBean(ProductServiceImpl.class);
        Product prodToBuy;
        // Try to decrement the product then send user to confirmation page
        try {
            prodToBuy = productService.findById(theID);
            int inv = prodToBuy.getInv();
            if(inv > 0) {
                prodToBuy.setInv(inv - 1);
        ...
        // If it fails due to server error or out of inventory, send user to error page
